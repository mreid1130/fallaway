...
post '/players/fallspace/update' do
	if session[:player_id]
		...
		if params[:score].to_i  > @player.high_fallspace_score
			@player.update_attribute(:high_fallspace_score, params[:score].to_i)
		end
...
post '/players/footballurrito/update' do
	if session[:player_id]
		...
		if params[:burritos].to_i > @player.high_burritos
			@player.update_attribute(:high_burritos, params[:burritos].to_i)
		end

		if params[:opponents].to_i  > @player.high_opponents
			@player.update_attribute(:high_opponents, params[:opponents].to_i)
		end

		if params[:score].to_i  > @player.high_footballurrito_score
			@player.update_attribute(:high_footballurrito_score, params[:score].to_i)
		end
